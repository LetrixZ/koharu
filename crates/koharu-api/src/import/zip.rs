...
mod tests {
    ...
    fn write_archive(entries: &[(&str, &[u8])]) -> Vec<u8> {
        ...
        let cursor = archive.finish().expect("finish archive");
        cursor.into_inner()
    }
    ...
    fn reads_images_in_natural_member_order_and_skips_metadata() {
        ...
        let images = extract(bytes, "koharu-import-cbz.cbz").expect("read archive");
        assert_eq!(
            images
                .iter()
    ...
    fn rejects_archive_without_supported_images() {
        let bytes = write_archive(&[("ComicInfo.xml", b"<ComicInfo />")]);
        let error =
            extract(bytes, "koharu-import-cbz.cbz").expect_err("archive should be rejected");
